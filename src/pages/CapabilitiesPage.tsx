  ...
  return (
    <>
      ...
      <div className="relative max-w-[1200px] mx-auto px-5 sm:px-8">
        ...
        >
          Four actions. One working system.
        </motion.h1>
        ...
        <div className="space-y-6">
          {CAPABILITIES.map((cap, i) => (
            <motion.div
              key={cap.id}
              id={cap.id}
              className="rounded-panel bg-carbon border border-hairline p-7 sm:p-10 scroll-mt-28"
            >
              <div className="grid lg:grid-cols-[1fr_1fr] gap-10 lg:gap-16">
                <div className="min-w-0">
                  <div className="flex items-baseline gap-4 mb-4">
                    <span className="font-mono text-sm text-lime tabular-nums">
                      {String(i + 1).padStart(2, '0')}
                    </span>
                    <h2 className="font-sans text-[clamp(28px,4vw,44px)] font-medium leading-tight tracking-[-0.02em]">
                      {cap.name}
                    </h2>
                  </div>
                  ...
                  <p className="text-[13px] text-fog mb-3">Systems</p>
                  <div className="flex flex-wrap gap-2">
                    {cap.systems.map((s) => (
                      <span key={s} className="rounded-panel border border-hairline text-fog text-[12px] px-3 py-1.5">
                        {s}
                      </span>
                    ))}
                  </div>
                ...
                <div className="min-w-0">
                  <p className="text-[13px] text-fog mb-5">What good looks like</p>
        ...
        <div className="pt-24">
          ...
          <motion.div
            className="rounded-panel bg-carbon border border-hairline p-7 sm:p-12"
          >
            <div className="mx-auto max-w-[560px] space-y-3">
              {[...CAPABILITIES].reverse().map((cap, i) => (
                <div
                  key={cap.id}
                  className="rounded-panel border px-5 py-4 flex items-center justify-between gap-4"
                  style={{
                    borderColor: `rgba(216,166,68,${0.4 - i * 0.09})`,
                    background: `rgba(216,166,68,${0.09 - i * 0.02})`,
                    marginLeft: `${i * 18}px`,
                    marginRight: `${i * 18}px`,
                  }}
                >
                  <span className="text-[15px] font-medium text-paper ">{cap.name}</span>
                  <span className="font-mono text-[11px] text-fog">{cap.systems.length} systems</span>
                </div>
              ))}
              <div className="rounded-panel bg-paper px-5 py-4 text-center">
